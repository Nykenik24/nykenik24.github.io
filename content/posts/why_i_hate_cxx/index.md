...
C++ is a _beast_, and I mean it. Like, include one header and `std` will be full of methods that you will probably never use.
...
This might seem like it's super cool, right? I mean, you don't have to do anything yourself! Yeah, not really. This just makes it:
...
In C, the same snippet (replacing it with the respective header in C, `stdio.h`) brings:
...
Each one brings has advantages and disavantages, may cause errors in some situations, does it's own things in the background, etc. Basically like choosing kart and character in mario kart, each combo has unique stats and tradeoffs.
...
Like, there are all of these features, abstractions, standard library headers, etc. and it just makes it so hard to make simple things without just writing C code inside C++ with slight modifications. And if I am going to basically write C then I will just use C.
...
For _one pointer_ 😭 (so bad I had to use an emoji to express my feelings).
...
C++ is a complex, bloated and abstract language.

Yes, there are some very nice projects made in C++, there are senior C++ developers that are so used to it's quirks they can completely
ignore them and even deal with them completely. But, if you are a normal programmer which wants to enjoy their development experience,
C++ will destroy your life and feelings, kidnap your pets and torture your family.

And, if you are a C++ developer and are angry about my post, I admit that some information might be exaggerated, but nobody can convince me
that some of these things are completely true. Plus you probably agree with some of these.
